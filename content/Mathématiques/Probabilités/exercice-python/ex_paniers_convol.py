...
def simu(nb_realisations,loi_x):
    X_cum=loi_x.copy()
    X_cum[1]=np.cumsum(X_cum[1])
    alea=np.random.rand(nb_realisations)
    simu_X=np.zeros(nb_realisations)
    for i in range(nb_realisations):
        for j in range(X_cum.shape[1]):
            if alea[i]<X_cum[1][j]:
                simu_X[i]=X_cum[0][j]
                break
    return simu_X
...
if __name__ == "__main__":
    ...
    print('estimation espérance H par simu : ',simu(1000,res_h).sum()/1000)
